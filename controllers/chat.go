...
var (
	chatKey     = ""
	chatWait    = ""
	redisConfig = *new(mdDB.RedisConfig)
)

func init() {
	utilsController := NewUtilsController()
	config := utilsController.GetConfig()
	chatKey = config.GetString("application.chat.chatKey")
	chatWait = config.GetString("application.chat.chatWait")
	redisConfig = mdDB.RedisConfig{
...
func NewChatController(socketTool *mdDB.RedisTool) *ChatController {
...
func (chatController ChatController) HandleMessage(socketTool *mdDB.RedisTool, mongoTool *mdDB.MongoTool) {
	...
	msgController := NewMessageController(mongoTool)
	...
	melodyNew.HandleMessage(func(session *melody.Session, msg []byte) {
		...
		newMsg := msgController.NewMessage(string(msg), utilsController.GetUsername(session))
...
func (chatController ChatController) HandleClose(socketTool *mdDB.RedisTool, mongoTool *mdDB.MongoTool) {
	...
	melodyNew.HandleClose(func(session *melody.Session, i int, s string) error {
		...
		newMsg := msgController.NewMessage(utilsController.GetUsername(session)+" left the room", "Server")
		msgController.InsertMessage(newMsg)
		msg, _ := json.Marshal(newMsg)
